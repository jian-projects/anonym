...
def config(task, dataset, model, opt=None, framework=None):
    ...
    args.file = {
        ...
        'data_dir': f"dataset/", # 数据路径
        'save_dir': f'checkpoints/{dataset}/', # 模型保存路径

        # 'cache_dir': cache_dir, # 缓存路径, 加快加载
    ...
    args.train['display'] = ['epochs', 'early_stop', 'batch_size', 'learning_rate_pre', 'seed']
